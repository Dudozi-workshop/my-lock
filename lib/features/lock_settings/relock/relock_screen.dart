...
class _RelockScreenState extends State<RelockScreen> {
  ...
  void initState() {
    super.initState();
    _selectedPolicy = widget.selectedPolicy;
  }
    ...
    return Scaffold(
      ...
      body: SafeArea(
        ...
        child: ListView(
          ...
          children: [
            Text(
              '잠금 해제 후 앱을 벗어났을 때 언제 다시 잠글지 설정합니다.',
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                    color: selected ? brandPurple : const Color(0xFFC9C6D0),
                    width: 1.5,
                  ),
                ),
                child: selected
  ...
  String _description(RelockPolicy policy) {
    switch (policy) {
      case RelockPolicy.immediate:
        return '보호 앱을 벗어나는 즉시 다시 잠급니다.';
      case RelockPolicy.after30Seconds:
        return '30초 안에 다시 열면 잠금 화면을 건너뜁니다.';
      case RelockPolicy.after1Minute:
        return '1분 안에 다시 열면 잠금 화면을 건너뜁니다.';
      case RelockPolicy.screenOff:
        return '화면을 끄기 전까지 잠금 해제 상태를 유지합니다.';
    }
  }
}
